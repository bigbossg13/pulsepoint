...
export function mergeTeams({ scoutTeams = [], officialTeams = [], attendance = new Map() }) {
  ...
  for (const t of scoutTeams) {
    ...
    team.city = pick(team.city, t.location?.city, t.city);
    team.state = pick(team.state, t.location?.state, t.state);
    team.country = pick(team.country, t.location?.country, t.country);
...
export function mergeEvents({ scoutEvents = [], officialEvents = [], today }) {
  ...
  for (const e of scoutEvents) {
    ...
    ev.city = pick(ev.city, e.location?.city, e.city);
    ev.state = pick(ev.state, e.location?.state, e.state);
    ev.country = pick(ev.country, e.location?.country, e.country);
